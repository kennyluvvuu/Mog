...
/**
 * GET /ratings
 * Получение истории оценок текущего пользователя с пагинацией
 */
  ...
  try {
    ...
    if (error) {
      return c.json(
        {
          error: "DatabaseError",
          message: `Ошибка получения истории оценок: ${error.message}`,
        },
        500
      );
    }

    // Генерируем временные signed URLs для фото
    ...
    return c.json({
      ...
      pagination: {
        limit,
        offset,
        total: count ?? itemsWithUrls.length,
      },
    });
  } catch (error) {
    return c.json(
...
/**
 * GET /ratings/:id
 * Получение детальных результатов конкретной оценки
 */
ratingsRouter.get("/:id", async (c) => {
  ...
  try {
    ...
    if (error) {
      return c.json(
        {
          error: "DatabaseError",
          message: `Ошибка запроса оценки: ${error.message}`,
        },
        500
      );
    }
    ...
    // Создаем временный Signed URL на просмотр фото
    let photoUrl: string | null = null;
